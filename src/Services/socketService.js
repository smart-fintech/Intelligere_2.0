...
/** Prints only while running `npm run dev`. A built site says nothing. */
const log = (text) => {
  if (ENV.IS_DEV) console.info(`[socket] ${text}`)
}
...
const notify = (listeners, value, label) => {
  listeners.forEach((listener) => {
    try {
      ...
      console.error(`[socket] a ${label} failed`, error)
    }
  })
}
...
const readMessage = (data) => {
  ...
  return data.res?.message ?? data.message ?? data
}

/** Which module a message belongs to, or null when it is for everybody. */
const readModule = (data) => readMessage(data).module ?? null
...
const handleCommonMessage = (data) => {
  const msg = readMessage(data).msg
  const handler = msg ? COMMON_MESSAGES[msg] : null
...
const flushQueue = () => {
  ...
  if (waiting > 0) log(`connected - sent ${waiting} message(s)`)
}
...
const dropQueue = () => {
  ...
  log(`could not deliver ${queue.length} message(s) - the server is not answering`)
 ...
 */
const openSocket = () => {
  if (!allowed) return // nobody is signed in
  ...
  socket.onmessage = (event) => {
    ...
    messageListeners.forEach((entry) => {
      ...
      try {
        ...
        console.error('[socket] a message listener failed', error)
      }
    })
  }

  // The browser's error event carries nothing but `{ isTrusted: true }`, and
  // onclose always follows it, so this stays empty on purpose.
  socket.onerror = () => { }
 ...
 */
export const sendAll = (payloads = []) => Promise.all(payloads.map((one) => send(one)))
 ...
 */
export const onMessage = (handler, { module = null, connectOnMount = false } = {}) => {
...
export const onOpen = (handler) => {
  ...
  if (status === 'open') {
    try {
      ...
      console.error('[socket] an open listener failed', error)
    }
  }
...
export const onBusyChange = (handler) => {
...
export const isSocketOpen = () => status === 'open'
